        ...
        for(const property in apiKeys) {
            ...
            request.onload = () => {
                ...
                if (request.status === 200) {
                    ...
                    if (getStatus === '0') {
                        var status = "mait";
                        $(`#${property}`).addClass("mait");
                        $(`#${property}`).removeClass("online");
                        $(`#${property}`).removeClass("offline");
                        $(`#${property}text`).removeClass("offlinetext");
                        $(`#${property}text`).removeClass("onlinetext");
                        $(`#${property}text`).addClass("maittext");
                    }
                    // If offline
                    if (getStatus === '8' || getStatus === '9') {
                        var status = "offline";
                        $(`#${property}`).addClass("offline");
                        $(`#${property}`).removeClass("mait");
                        $(`#${property}`).removeClass("online");
                        $(`#${property}text`).removeClass("maittext");
                        $(`#${property}text`).removeClass("onlinetext");
                        $(`#${property}text`).addClass("offlinetext");
                    }
                    // If online
                    if (getStatus === '2') {
                        var status = "online";
                        $(`#${property}`).addClass("online");
                        $(`#${property}`).removeClass("offline");
                        $(`#${property}`).removeClass("mait");
                        $(`#${property}text`).removeClass("maittext");
                        $(`#${property}text`).removeClass("offlinetext");
                        $(`#${property}text`).addClass("onlinetext");
                    }
